...
final class MultipleAssignmentDeclarationTest {
    ...
    // GEP-20 empty rest mutability: when the rest binding captures zero elements, the
    // result must be a mutable list — not Collections.emptyList() — so that downstream
    // mutation (`rest << x`, `rest.add(x)`) behaves the same whether the rest absorbed
    // zero or N elements. Mirrors Groovy's getAt(EmptyRange) contract on List/array.

    @Test
    void testTailRest_emptyRest_isMutable_listRhs() {
        assertScript '''
            def (a, *t) = [1]
            assert t == []
            t << 2
    ...
    @Test
    void testTailRest_emptyRest_isMutable_arrayRhs() {
        assertScript '''
            Integer[] arr = [1, 2, 3] as Integer[]
            def (a, b, c, *t) = arr
            assert t == []
    ...
    @Test
    void testMiddleRest_emptyMiddle_isMutable_listRhs() {
        assertScript '''
            def (l, *m, r) = [10, 20]
            assert m == []
            m << 15
    ...
    @Test
    void testMiddleRest_emptyMiddle_isMutable_arrayRhs() {
        assertScript '''
            Integer[] arr = [10, 20] as Integer[]
            def (l, *m, r) = arr
            assert m == []
    ...
    @Test
    void testTailRest_emptyRest_preservesListSubtype() {
        assertScript '''
            def src = new LinkedList<Integer>([1])
            def (a, *t) = src
            assert t == []
            assert t instanceof LinkedList
            t << 2
            assert t == [2]
        '''
    }
